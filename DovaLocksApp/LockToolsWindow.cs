...
public sealed class LockToolsWindow:Window
{
 ...
 public LockToolsWindow()
 {
  ...
  toolbar.Children.Add(Action("Choose lock save",Choose));toolbar.Children.Add(Action("Export editable copy",Export));toolbar.Children.Add(Action("Import edited copy",Import));
  head.Children.Add(world);
 ...
 void Working(bool value){busy=value;progress.Visibility=value?Visibility.Visible:Visibility.Collapsed;workspace.IsEnabled=!value;foreach(var b in actions)b.IsEnabled=!value;apply.IsEnabled=!value&&dirty;}
 ...
 void RefreshList(){int index=locks.SelectedIndex;locks.Items.Clear();if(document==null)return;foreach(var l in document.Locks)locks.Items.Add(new ListBoxItem{Content=(l.RemoveLock?"[Clear on save] ":l.Pin==""?"[Unlocked] ":"")+(string.IsNullOrEmpty(l.Owner.Name)?"Unnamed owner":l.Owner.Name)+"\n"+l.Id,Padding=new Thickness(8),ToolTip=l.Id});if(locks.Items.Count>0)locks.SelectedIndex=Math.Clamp(index,0,locks.Items.Count-1);}
 void Changed(){dirty=true;apply.IsEnabled=true;status.Text="Changes are pending. Save when you're ready.";}
 void ShowLock()
 {
  details.Children.Clear();if(document==null||locks.SelectedIndex<0)return;int index=locks.SelectedIndex;var l=document.Locks[index];
 ...
 void Export(object s,RoutedEventArgs e){if(document==null){status.Text="Choose a lock save first.";return;}var d=new SaveFileDialog{Filter="Editable lock file|*.json",FileName="Dova-Locks-editable.json"};if(d.ShowDialog(this)!=true)return;try{File.WriteAllText(d.FileName,JsonSerializer.Serialize(document,LockSaveEditor.JsonOptions));status.Text="Exported. The editable copy contains PINs and Steam IDs, so keep it private.";}catch(Exception ex){status.Text=ex.Message;}}
 void Import(object s,RoutedEventArgs e){if(save==null){status.Text="Choose a lock save first.";return;}if(!Discard())return;var d=new OpenFileDialog{Filter="Editable lock file|*.json"};if(d.ShowDialog(this)!=true)return;try{var edited=JsonSerializer.Deserialize<EditableLocks>(File.ReadAllText(d.FileName),LockSaveEditor.JsonOptions)??throw new IOException("Empty edit file.");LockSaveEditor.ApplyEdits(LockSaveEditor.Latest(save),edited);document=edited;Changed();RefreshList();}catch(Exception ex){status.Text=ex.Message;}}
 async void Apply(object s,RoutedEventArgs e)
 {
  if(save==null||document==null||!dirty)return;
  if(MessageBox.Show(this,"Save these changes to both A and B? Stop the game or server first. Backups will be created before writing.","Save world locks",MessageBoxButton.OKCancel,MessageBoxImage.Question)!=MessageBoxResult.OK)return;
  Working(true);status.Text="Backing up and saving both copies...";
  try {foreach(var game in InstallCore.Detect())InstallCore.EnsureGameClosed(game.Root);var path=save;var edited=document;string backup=await Task.Run(()=>LockSaveEditor.ImportDocument(path,edited));dirty=false;await LoadSave(path);status.Text="Both saves updated. Upload both to your hosted server. Backups: "+backup;}
  catch(Exception ex){status.Text="Nothing confirmed saved: "+ex.Message;}
  finally{Working(false);}
 }
}

...
public static class NetworkTests
{
 public static async Task Run()
 {
  ...
  foreach(var tag in new[]{"v1.0.6"})
  {
   var bundled=ReleaseClient.SelectLatest(JsonSerializer.Serialize(new {tag_name=tag,draft=false,prerelease=false,assets=Array.Empty<object>()}));
   var payload=await new ReleaseClient().Download(bundled);
   if(bundled.Version!=InstallCore.Release||bundled.Hash!=InstallCore.PayloadHash||payload.Length==0)throw new Exception("Older or pending publication did not retain current bundle");
  }
  Console.WriteLine("PASS: older public release installs the verified current bundle");
  var same=ReleaseClient.SelectLatest(Json().Replace("9.8.7",InstallCore.Release));
  if(same.Url=="embedded"||same.Hash!=hash)throw new Exception("Same-version GitHub correction was hidden by bundled PAK");
  Console.WriteLine("PASS: same-version release uses verified GitHub PAK instead of stale bundle");
  ...
  Console.WriteLine("PASS: selected release "+latest.Version+" ("+latest.Url+") verified and installed into the selected isolated fixture; installed file verified; no real game files changed");
 }
